...
func (m *mongoSchema) toEventSchema() *evtschema.EventSchema {
...
func fromEventSchema(s *evtschema.EventSchema) *mongoSchema {
	m := &mongoSchema{
		Name:              s.Name,
		Version:           s.Version,
...
type MongoProvider struct {
	...
	publisher  func(context.Context, evtschema.SchemaChangeEvent) error
	watchers   []chan evtschema.SchemaChangeEvent
	mu         sync.RWMutex
	closed     bool
...
// The publisher callback is called when a schema is set; it can be nil if Watch is not needed.
func NewMongoProvider(db *mongo.Database, publisher func(context.Context, evtschema.SchemaChangeEvent) error) (*MongoProvider, error) {
	if db == nil {
...
func (p *MongoProvider) Get(ctx context.Context, eventName string) (*evtschema.EventSchema, error) {
	p.mu.RLock()
	if p.closed {
		p.mu.RUnlock()
		return nil, evtschema.ErrProviderClosed
	}
	p.mu.RUnlock()
...
func (p *MongoProvider) Set(ctx context.Context, schema *evtschema.EventSchema) error {
	...
	if p.closed {
		p.mu.RUnlock()
		return evtschema.ErrProviderClosed
	}
	p.mu.RUnlock()
	...
	if existing != nil && schema.Version < existing.Version {
		return fmt.Errorf("%w: %d < %d", evtschema.ErrVersionDowngrade, schema.Version, existing.Version)
	}
	...
	change := evtschema.SchemaChangeEvent{
...
func (p *MongoProvider) Watch(ctx context.Context) (<-chan evtschema.SchemaChangeEvent, error) {
	p.mu.Lock()
	defer p.mu.Unlock()
	...
	ch := make(chan evtschema.SchemaChangeEvent, 100)
...
func (p *MongoProvider) List(ctx context.Context) ([]*evtschema.EventSchema, error) {
	p.mu.RLock()
	if p.closed {
		p.mu.RUnlock()
		return nil, evtschema.ErrProviderClosed
	...
	var schemas []*evtschema.EventSchema
	for cursor.Next(ctx) {
		var m mongoSchema
...
// Compile-time check
var _ evtschema.SchemaProvider = (*MongoProvider)(nil)

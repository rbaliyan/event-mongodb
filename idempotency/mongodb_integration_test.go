...
// unavailable or when -short is set. Teardown (store close + db drop) is
// registered via t.Cleanup so it runs even if a test panics.
func setupIdempotencyIntegrationTest(t *testing.T) (*MongoStore, func()) {
	t.Helper()

	client := mongotest.Connect(t)

	db := client.Database(mongotest.UniqueDBName("test_event_mongodb_idem"))
...
func TestIntegration_IsDuplicate_FirstThenSecond(t *testing.T) {
	store, _ := setupIdempotencyIntegrationTest(t)
